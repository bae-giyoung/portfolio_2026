...
export const projectDetails: ProjectDetail[] = [
...

    {
        ...
        impact: [
            ...
            "GSAP 커튼 애니메이션 속성 개선(height → scaleY, top → yPercent)으로 CLS 1.0 → 0 개선",
        ...
        technicalChallenges: [
            ...
            {
                title: "GSAP 커튼 애니메이션 CLS 개선",
                problem: [
                    "커튼 전환 애니메이션에 height와 top 속성을 직접 변경하는 방식을 사용하자 브라우저가 레이아웃을 재계산하여 CLS(Cumulative Layout Shift)가 1.0으로 측정됨",
                ],
                solution: [
                    "height 애니메이션을 scaleY로 전환해 레이아웃 재계산 없이 GPU 합성 레이어에서 처리 → CLS 1.0 → 0.4~0.53으로 감소",
